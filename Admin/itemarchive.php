...
$i_name=$i_price=$i_stock=$i_image=$varunit=$i_Original="";
$i_nameErr=$i_priceErr=$i_stockErr=$i_imageErr=$varunitErr=$i_OriginalErr="";
...
if(isset($_POST['btnsubmit'])){
	...
	//
	if(empty($_POST["i_Original"])){
		$i_OriginalErr="Original is required !";
	}else{
		$i_Original=$_POST["i_Original"];
	}
	////
 ...
 date_default_timezone_set('Asia/Manila');
	...
	$check_product = mysqli_query($conn,"SELECT * from table_product where item_name='$i_name'");
		...
		if($check_product_row  > 0){
		 ...
		 mysqli_query($conn, "INSERT INTO `table_product`(`item_name`,unit,`orig_price`,`price`,`stocks`, `image`,`date_added`) VALUES ('$i_name','$varunit','$i_Original','$i_price','$i_stock','$p_image','$day,$date')") or die('query failed');
...
<button class="btn btn-primary" name="search"><h1>SEARCH</h1><span class="glyphicon glyphicon-search"></span></button>
</span>
              ...
              <div>
                <div class="his1">
                <h2 class="tran1">Item Archive</h2>
...
<?php $view_query = mysqli_query($conn,"SELECT * from table_product where product_status='1' "); 
		...
		while($row = mysqli_fetch_assoc($view_query)){ //<-- ginagamit tuwing kukuha ng database
			...
			?>
                                      <tr>
                                        ...
                                        <td>₱ <?php echo number_format($db_orig_price,2)?> </td>
                                        <td>₱ <?php echo number_format($db_price)?>  </td>
                                       
                                      
                                        <td> 
	
                                            <a href="restore_item.php?deleteid=<?php echo $db_item_number?>"><button class="e">ITEM ARCHIVE</button></a>
	<?php } ?>
      ...
      <div class="right">
        ...
        <form method="POST" enctype="multipart/form-data">
           <h2 class="tran2">Add New Items</h2><br><br><br><br><br><br><br>
				...
				<br>
				<label class="ad">Original Price</label>
				
				<br>
                <input value="<?php echo $i_Original ?>" class="b" type="number" name="i_Original"><br> 
				
			<center>	<b><span class="error"><?php echo $i_OriginalErr; ?></span></b></center>
				<Br>
                <label class="ad">Current Price</label><br> 
               <input value="<?php echo $i_price ?>"  class="b" value="" placeholder="" type="number" name="i_price"><br>  
			<center>	<b><span class="error"><?php echo $i_priceErr; ?></span></b></center>
				<Br>
                <label class="ad">Stock</label><br> 
			
                <input  value="<?php echo $i_stock?>"  class="b" type="number" name="i_stock" value="" placeholder=""><br> 
