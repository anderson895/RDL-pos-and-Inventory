...
if(isset($_POST["save"])){
	...
	if(empty($_POST["new_tax"])){
		
		$new_taxErr="Input Value of Tax !";
	}else{
 ...
 date_default_timezone_set('Asia/Manila');
	...
	if(is_numeric($new_tax)){	
	
	
	mysqli_query($conn,"UPDATE `system_maintinance` SET `tax`='$new_tax',`date_edit`='$day,$date' WHERE system_id='4'");
//	mysqli_query($conn,"UPDATE `system_maintinance` SET `tax` = '10' WHERE `system_maintinance`.`system_id` = 4;");
...
echo "<script>alert('Update Tax Successfully .');
	...
	$new_taxErr=" Enter Numeric Or Decimal Value Only !";
}
	}
...
<form method="post">
      ...
      <section>
        <div class="left">
                ...
                <h2 class="lp">CHANGE TAX VALUE</h2>
                <br><br><br><br><br><br><br><br><br>
                <label>Enter New Tax Value</label><br><br><br>
                <input type="text" name="new_tax" placeholder=" <?php
