...
<center>
  ...
  <li><a class="active" class="a" href="settings.php">
      ...
      <section>
        <div class="left">
				...
				if(isset($_POST["btnSubmit"])){
					...
					if(empty($_POST["newUsername"])){
						...
						$newUsernamerr="Username is Required";
		...
		$userID=$_GET["userID"];
			$view_query = mysqli_query($conn,"SELECT * from user where user_id ='$newUsername' "); 
		...
		while($row = mysqli_fetch_assoc($view_query)){ //<-- ginagamit tuwing kukuha ng database
			...
			// echo $ic; 
				...
				if(strlen($newUsername)>0){
					...
					$check_username = mysqli_query($conn,"SELECT * from user WHERE username='$newUsername'");
		$check_username_row = mysqli_num_rows ($check_username);
		
		if($check_username_row  > 0){
			 echo "<script>alert('Error Username is Already Exist .');</script> ";
		}else{			
		 mysqli_query($conn,"UPDATE user SET username='$newUsername' where user_id ='$db_user_id' ");
		
	   
	  echo "<script>alert('Update Username Successfully .');
	...
	window.location='settings.php';
					...
					$newUsernameErr="Username is too short ! ";
				}
				...
				<form method="POST">
				<br>
				 <center> New USername<br>
				<input type="text" name="newUsername" value="<?php echo $newUsername ;?>" placeholder="INPUT NEW USERNAME" ></input>
				
              
                <button type="submit" name="btnSubmit" class="set">SUBMIT</button><br>
                <button class="set" onclick="window.location.href='settings.php';"><a href="settings.php">CANCEL</a></button><br>
                </form>
                      </div> 
              </div>
